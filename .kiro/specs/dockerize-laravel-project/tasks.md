...
- [x] 2. PHP-FPM konteyner yapılandırmasını oluştur
  - [x] 2.1 PHP-FPM Dockerfile dosyasını oluştur (`docker/php/Dockerfile`)
  ...
  - [x] 2.2 PHP yapılandırma dosyasını oluştur (`docker/php/php.ini`)
  ...
  - [x] 2.3 PHP-FPM pool yapılandırmasını oluştur (`docker/php/www.conf`)
    - Dynamic process manager ayarları
    - Health check için ping.path ve status_path yapılandırması
    - _Gereksinimler: 1.1, 10.1_

- [x] 3. Nginx web sunucusu yapılandırmasını oluştur
  - [x] 3.1 Nginx ana yapılandırma dosyasını oluştur (`docker/nginx/nginx.conf`)
  ...
  - [x] 3.2 Varsayılan site yapılandırmasını oluştur (`docker/nginx/conf.d/default.conf`)
  ...
  - [x] 3.3 SSL yapılandırma dosyasını oluştur (`docker/nginx/conf.d/ssl.conf`)
...
- [x] 4. MySQL ve Redis yapılandırmasını oluştur
  - [x] 4.1 MySQL yapılandırma dosyasını oluştur (`docker/mysql/my.cnf`)
    - character-set-server = utf8mb4
    - collation-server = utf8mb4_unicode_ci
    - InnoDB buffer pool ve bağlantı limitleri
    - _Gereksinimler: 3.3, 3.4_

- [x] 5. Checkpoint - Tüm yapılandırma dosyalarını kontrol et
  - Tüm dosyaların doğru dizinlerde oluşturulduğunu doğrula, kullanıcıya sor.

- [x] 6. Docker Compose dosyalarını oluştur
  - [x] 6.1 docker-compose.yml dosyasını oluştur (geliştirme ortamı)
  ...
  - [x] 6.2 docker-compose.prod.yml dosyasını oluştur (üretim override)
...
- [x] 7. Ortam ve yardımcı dosyaları oluştur
  - [x] 7.1 .env.docker örnek dosyasını oluştur
  ...
  - [x] 7.2 Makefile dosyasını oluştur
...
- [x] 8. Checkpoint - Docker yapısını doğrula
  - Tüm dosyaların oluşturulduğunu ve birbiriyle uyumlu olduğunu doğrula, kullanıcıya sor.

- [x] 9. Test ve doğrulama
  - [x] 9.1 Smoke test komutlarını doğrula
    - `docker compose config` ile YAML geçerliliğini kontrol et ✅
    - Dockerfile syntax kontrolü ✅
    - Tüm Docker dosyalarının varlık kontrolü ✅
    - _Gereksinimler: 5.1, 5.4_

  - [x] 9.2 Entegrasyon test doğrulaması
    - Tüm servisler tanımlı: app, nginx, mysql, redis ✅
    - Tüm servislerde healthcheck mevcut ✅
    - Volume'lar doğru yapılandırılmış (mysql_data, redis_data, vendor_data, storage_data, certbot_conf, certbot_www) ✅
    - app-network bridge ağı tanımlı ✅
    - Restart politikaları (unless-stopped) tüm servislerde aktif ✅
    - depends_on + service_healthy koşulları doğru ✅
    - Production compose (certbot dahil) geçerli ✅
    - _Gereksinimler: 3.7, 4.4, 5.4, 10.1, 10.2_

- [x] 10. Son checkpoint - Tüm dosyaları ve yapılandırmaları doğrula
  - Tüm Docker dosyaları mevcut ve geçerli ✅
  - docker compose config başarılı ✅
  - docker-compose.prod.yml override geçerli ✅
